...
const double MathTool::kEpsilon = 0.0001;
...
// 计算点到直线距离
///
double MathTool::ComputeDistancePointToLine(const float line_vec[3],
...
{ 
	...
	float dir[3];
	for(i=0; i<3; ++i){
		dir[i] = point_outside[i] - point_on_line[i];
	}
	float dot = line_vec[0] * dir[0] + line_vec[1] * dir[1] + line_vec[2] * dir[2];
	float distance = sqrt(dir[0] * dir[0] + dir[1] * dir[1] + dir[2] * dir[2] - dot * dot);
