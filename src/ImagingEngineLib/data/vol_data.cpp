...
VolData::VolData()
{
	...
	mark_bounding_box_clipping_ = new BoundingBox();
	slice_width_ = 512;
	slice_height_ = 512;
	series_mark_ = NULL;
	pixel_data_ = NULL;
...
void VolData::SetMarkBoundingBoxClipping(float xmin,float ymin,float zmin,float xmax,float ymax,float zmax)
{
	mark_bounding_box_clipping_->Update(xmin, ymin, zmin, xmax, ymax, zmax);
}
