...
namespace DW {
	namespace Render {
		...
		public:
			...
			---------------------End--------------------*/
			/// Defineix expl¨ªcitament el punt (x,(color,opacity)).
			void SetColorOpacity(double x, const Color4 &color, double opacity);
			
			/// Returns the color transfer function in VTK format.
			vtkColorTransferFunction* TovtkColorTransferFunction() const;
			...
			TransferFunction ToNewRange(double oldX1, double oldX2, double newX1, double newX2);
			...
			/// Clear old color, opacity and gradient data
			void ClearAll();
			/// Clear old color and opacity data
			void ClearColor();
			/// Clear old color and opacity data
			void ClearOpacity();
